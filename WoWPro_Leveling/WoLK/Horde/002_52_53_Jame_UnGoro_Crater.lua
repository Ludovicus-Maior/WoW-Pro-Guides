WoWPro_Leveling:RegisterGuide("JamUng5253", "Un'Goro Crater", "Jame", "52", "53", "JamAzs5354", "Horde", function()
return [[

H Orgrimmar|QID|936|U|6948|
A Assisting Arch Druid Runetotem|QID|936|N|From the Inkeeper.|
A Bone-Bladed Weapons|QID|4300|N|The top floor of the alchemy building in The Drag.|M|55.6,34.1|

F Thunder Bluff|QID|936|
T Assisting Arch Druid Runetotem|QID|936|
A Un'Goro Soil|QID|3761|M|78.6,28.5|

F Gadgetzan|QID|4494|
A Super Sticky|QID|4504|M|51.6,26.8|
T March of the Silithid|QID|4494|M|50.9,27.0|
A Bungle in the Jungle|QID|4496|M|50.9,27.0|

R Un'Goro Crater|QID|4289|N|Head west to the slope leading down into Un'Goro .|Z|Tanaris|M|27,57|
A The Apes of Un'Goro|QID|4289|M|71.6,76.0|
A The Fare of Lar'korwi|QID|4290|M|71.6,76.0|

N Things to do|QID|3844|N|Collect crystals, Un'Goro soil, and kill raptors as you go.|
A It's a Secret to Everybody (Part 1)|QID|3844|N|From a wrecked raft at |M|63.1,68.5|
T It's a Secret to Everybody (Part 1)|QID|3844|N|Underwater, the pack is at .|M|63.08,69.03|
A It's a Secret to Everybody (Part 2)|QID|3845|M|63.1,69.1|

C The Fare of Lar'korwi|QID|4290|N|Click the Fresh Threshadon Carcass at |M|68.8,56.7|
T The Fare of Lar'korwi|QID|4290|M|71.6,76.0|
A The Scent of Lar'korwi|QID|4291|M|71.6,76.0|

C The Scent of Lar'korwi|QID|4291|N|Go to  and clear the immediate area. Stand on the egg pile there, a Lar'korwi mate should appear. Kill her. Go to (66.00,66.72) and do the same to those eggs. Go back and forth until you get two Ravasaur Pheromone Glands from the mates.|M|67.32,73.05|

T The Scent of Lar'korwi|QID|4291|M|71.6,76.0|

C Bone-Bladed Weapons|QID|4300|N|Kill raptors around  and gather power crystals.|M|67,73|
N Gather Blue Power Crystals|QID|4284|L|11184 7|N|Gather 7 power crystals of each color, as you kill Ravasaurs.|
N Gather Green Power Crystals|QID|4284|L|11185 7|N|Gather 7 power crystals of each color, as you kill Ravasaurs.|
N Gather Red Power Crystals|QID|4284|L|11186 7|N|Gather 7 power crystals of each color, as you kill Ravasaurs.|
N Gather Yellow Power Crystals|QID|4284|L|11188 7|N|Gather 7 power crystals of each color, as you kill Ravasaurs.|
N Mangled Journal|QID|3884|L|11116|N|Keep killing Ravasaurs until you find a Mangled Journal.|
A Williden's Journal|QID|3884|U|11116|N|Right-click the journal to accept the quest.|M|68.0,76.0|

A Chasing A-Me 01 (Part 1)|QID|4243|M|46.4,13.5|
R Marshal's Refuge|QID|3884|N||M|45.34,11.99|
A Shizzle's Flyer|QID|4503|M|44.2,11.6|
A Beware of Pterrordax|QID|4501|M|43.6,8.4|
A Roll the Bones|QID|3882|M|43.6,7.3|
A Alien Ecology|QID|3883|M|43.9,7.2|
A Expedition Salvation|QID|3881|M|43.9,7.2|
T Williden's Journal|QID|3884|M|43.9,7.2|
A Crystals of Power|QID|4284|M|41.9,2.7|
T Crystals of Power|QID|4284|M|41.9,2.7|
A The Eastern Pylon|QID|4287|M|41.9,2.7|
A The Northern Pylon|QID|4285|M|41.9,2.7|
A The Western Pylon|QID|4288|M|41.9,2.7|
f Marshal's Refuge|QID|3845|N||M|45.2,5.9|
A Larion and Muigin|QID|4145|M|45.5,8.7|
U Open the Small Pack|QID|3845|U|11107|
T It's a Secret to Everybody (Part 2)|QID|3845|M|44.7,8.1|
A It's a Secret to Everybody (Part 3)|QID|3908|M|44.7,8.1|
N Destroy Daggers and Photograph|QID|3908|N|You can destroy the throwing daggers and faded photograph from your inventory.|

N Kill on sight|QID|4501|N|Bloodpetals of any kind, Pterrodaxes, and Diametradons. Also watch out for Tyrant Devilsaurs.|
C The Northern Pylon|QID|4285|N|Right-click the Northern Crystal Pylon at |M|56.6,12.4|
T Chasing A-Me 01 (Part 1)|QID|4243|N|Kill gorillas on the way in.|M|67.6,16.8|
C The Apes of Un'Goro|QID|4289|N|Kill gorillas.|M|66.0,16.0|
C Super Sticky|QID|4504|N|Kill the tar monsters around .|M|60,26|
N Crate of Foodstuffs|QID|3881|QO|Crate of Foodstuffs: 1/1|N|Loot the Crate of Foodstuffs .|M|68.5,36.6|
C The Eastern Pylon|QID|4287|N|Right-click the Eastern Crystal Pylon at .|M|77.2,50.0|
T The Apes of Un'Goro|QID|4289|M|71.6,76.0|
C Alien Ecology|QID|3883|U|11132|N|Follow the arrows to the circular room, then use the Scraping Vial. (49.5,82.7)(48.58,85.49)|M|50.5,78.5|
N Gorishi Scent Gland|QID|4496|QO|Gorishi Scent Gland: 1/1|N|Kill bugs until one of these drops, inside or outside of the cave.|
K Pterrordax|QID|4501|QO|Pterrordax slain: 10/10|N|Follow the arrows two the three areas in the mountains, killing Pterrordax, then stay in the last area (if you need to) and kill Pterrordax until you have killed 10. (56.1,90.1)(50.5,90.1)(44.6,80.5)|M|21.0,59.0|
C Expedition Salvation|QID|3881|N|Right-click the Research Equipment at .|M|38.5,66.1|
A Finding the Source|QID|974|N|Watch out for the Devilsaur that roams around here.|M|30.9,50.4|
C The Western Pylon|QID|4288|N|Right-click the Western Crystal Pylon at |M|23.8,59.1|
C Larion and Muigin|QID|4145|N|Lashers and Threshers are in the north-east, Trappers to the west, and Flayers to the north.|

T Alien Ecology|QID|3883|M|43.9,7.2|
T Expedition Salvation|QID|3881|M|43.9,7.2|
T Larion and Muigin|QID|4145|M|45.5,8.7|
A Marvon's Workshop|QID|4147|M|45.5,8.7|
T The Northern Pylon|QID|4285|M|41.9,2.7|
T The Eastern Pylon|QID|4287|M|41.9,2.7|
T The Western Pylon|QID|4288|M|41.9,2.7|
A Making Sense of it|QID|4321|M|41.9,2.7|
T Making Sense of it|QID|4321|M|41.9,2.7|

F Gadgetzan|QID|4504|N||M|45.2,5.9|
T Super Sticky|QID|4504|M|51.6,26.8|
T Bungle in the Jungle|QID|4496|M|50.9,27.0|

F Thunder Bluff|QID|3761|N||Z|Tanaris|M|51.6,25.5|
T Un'Goro Soil|QID|3761|M|77.4,22.0|
A Morrowgrain Research (Part 1)|QID|3782|M|78.6,28.5|
T Morrowgrain Research (Part 1)|QID|3782|M|71.1,34.2|
A Morrowgrain Research (Part 2)|QID|3786|
N Evergreen Pouch|QID|3786|U|11020|N|Use your Evergreen Pouch every time it's off cooldown from now on.|

F Orgrimmar|QID|4300|N||Z|Thunder Bluff|M|46.8,50.2|
T Bone-Bladed Weapons|QID|4300|M|55.6,34.1|

]]
end)