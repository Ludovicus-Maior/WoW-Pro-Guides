...
function WoWPro_Leveling.UpdateGuideList()
	...
	for i,row in ipairs(rows) do
		...
		if iGuide then
			...
			if WoWPro_LevelingDB.currentguide == GID then
				row:SetChecked(true)
			else
		...
		local function OnClick()
			if IsShiftKeyDown() then
				local iGuide = guidelist[row.i]
				WoWPro_LevelingDB.currentguide = iGuide["GID"]
				WoWPro_LevelingDB[iGuide["GID"]] = nil
...
frame:SetScript("OnShow", function()
	local title, subtitle = LibStub("WoWPro-Heading").new(frame, "WoW-Pro Leveling - "..L["Guide List"], L["Available WoW-Pro leveling guides are listed below. \nSelect one and hit \"Okay\" to load. \nShift+click a guide to clear it."])
